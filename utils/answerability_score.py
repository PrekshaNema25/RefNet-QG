...
def NER_line(question):
    ...
    temp_words = []
    for i in question_words:
        if i in global_ner_dict and i not in stop_words:
            temp_words.append(i)
...
def _get_json_format_qbleu(lines, output_path_prefix, relevant_words=None, questiontypes=None):
    ...
    """
    return (data_pred_impwords, data_pred_ner, data_pred_qt, data_pred, data_pred_sw)


def loadJsonToMap(data):
...
def reinforce_compute_score(Z, data_type = "squad", save_to_files=False, 
                              delta=0.7, ner_weight = 0.41, qt_weight = 0.20, re_weight =0.36 , nist_meteor_scores_dir = "", output_dir="."):
     ...
     hypotheses = Z[0]
     references = Z[1]
     #hypotheses = [np.trim_zeros(i.flatten(), 'b') for i in hypotheses]
     #references = [np.trim_zeros(i.flatten(), 'b') for i in references]
     #for lines in hypotheses:
     #    word_hyp_lines.append(" ".join([word_to_index[i] for i in lines]))

     #for lines in references:
     #    word_ref_lines.append(" ".join([word_to_index[i] for i in lines]))

     new_score = get_answerability_scores(hypotheses = hypotheses, 
                              ...
                              qt_weight = qt_weight,
                              re_weight = re_weight, 
                              nist_meteor_scores_dir = nist_meteor_scores_dir,
                              output_dir = output_dir,
                              ngram_metric = 'Bleu_4')
     return new_score[0]

def reinforce_compute_score_parallel(hyp_scores, ref_scores):

   hyp_temp = [[i] for i in hyp_scores]
   ref_temp = [[i] for i in ref_scores]
   Z = list(zip(hyp_temp, ref_temp))
   #Z = list(zip(hyp_scores, ref_scores))
   p = Pool(32) 
   values = p.map(reinforce_compute_score, Z)
   p.terminate()
...
def main():
    ...
    print(np.mean(reinforce_compute_score_parallel( hypothesis_lines,
                             references_lines)))
    """
