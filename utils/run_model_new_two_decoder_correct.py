...
from dataset_iterator_two_decoder import *
from final_model_new_beam_two_decoder_correct import *
from postprocess_scripts.plt_attention import save_plot
from make_model_post_first_decoder import *
import os
import configparser
import pickle
import subprocess
...
def count_parameters():
    ...
    print ("--- Variable List ---", tf.__version__)
    total_parameters = 0
...
class run_model:
    ...
    def run_epoch(self, epoch_number, sess, writer,merge,fp = None):
        ...
        for step in xrange(steps_per_epoch):
            ...
            if epoch_number >= int(self.config["Hyperparams"]["epoch_before_rl"]) and (self.config["Hyperparams"]["rl_plus_crossent"] == "True" or self.config["Hyperparams"]["use_only_rl"] == "True"):
                ...
                bs = 0
                bg = 0
                _, loss_value,summary,outputs,coverage_loss, property_coverage_loss, prev_decoder_prop,question_loss, loss_mutual_information, masked_weights_p  = sess.run([self.train_op_crossent, self.loss_op, merge,
                                                                self.logits,self.coverage_loss,self.property_coverage_loss, self.loss_prev_decoder_prop, self.loss_question_label, self.loss_mutual_information, self.model.model.masked_weights_props] , feed_dict = feed_dict)
                loss_reinforce = -10.0
    ...
    def print_titles_in_files(self, sess, data_set, epoch = 1000):
        ...
        for step in xrange(steps_per_epoch):
            ...
            _decoder_states_ , attention_weights,attention_weights_prop,prev_decoder_states,prev_attention_weights,attention_weights_different, output_switch= sess.run([self.logits,
            self.attention_weights,self.attention_weights_prop,self.prev_decoder_logits,self.prev_attention_weights,self.attention_weights_different, self.output_switch], feed_dict=feed_dict)
    ...
    def run_training(self):
        ...
        with tf.Graph().as_default():
            ...
            if (os.path.exists(os.path.join(self.config["Log"]["output_dir"] , "best_model.meta"))):
                ...
                best_test_bleu = 0.0
                best_test_bleu, best_test_bleu_prev = 0, 0 #self.print_titles_in_files(sess, self.dataset.datasets["test"])
            ...
            if (os.path.exists(os.path.join(self.config["Log"]["output_dir"],  "best_model.meta"))):
                ...
                sess.run(init)
                saver.save(sess, os.path.join(self.config["Log"]["output_dir"] , "initial_params"))
                best_test_bleu = 0
    ...
    def run_inference(self):
        ...
        with tf.Graph().as_default():
            ...
            """
            final_outputs  = self.model.pipeline(self.encoder_input_batch, self.query_input_batch, self.decoder_input_batch,
                                                   self.feed_previous_placeholder,self.feed_true_placeholder,self.is_sample_placeholder)


            self.logits, self.attention_weights, self.logits_pos = final_outputs.get("outputs"), final_outputs.get("attention_weights"), final_outputs.get("pos_outputs")
            self.beta_weights, self.property_attention_weights = final_outputs.get("beta_values"), final_outputs.get("property_attention_weights")
            self.logits_rein = final_outputs.get("outputs_reinforce")
            self.bleu_greedy = final_outputs.get("bleu_crossent")
            self.bleu_sample = final_outputs.get("bleu_reinforce")
            self.predictions_reinforce = final_outputs.get("predictions_reinforce")
            self.question_logits = final_outputs["question_logits"]
            self.initial_states = final_outputs["initial_states"]
            self.new_state_values = final_outputs["new_state_values"]
            print ("LOGITS", self.logits, self.logits_pos)
            """        
            ...
            self.memory_states, self.prev_time_step_data, self.query_state,self.masked_weights,self.content_weights, self.post_first_decoder_pl, self.prev_attention_weights  = self.add_placeholders_beamsearch(self.encoder_input_batch,
            ...
            self.query_state_batch = tf.zeros(shape=(batch_size, 2*int(self.config["Query"]["hidden_size"])), dtype=tf.float32)
            self.prev_decoder_states, self.masked_weights_props, self.prev_decoder_mean_attn, self.total_encoder_teacher_attention_weights, self.question_gate, _ = self.model.post_first_decoder(self.post_first_decoder_pl, tf.unstack(self.prev_attention_weights,axis=1), self.query_state_batch)
            
            #self.initial_states_2nd, self.prev_decoder_states, self.prev_decoder_seq_length, self.masked_weights_props = self.model.post_first_decoder(self.post_first_decoder_pl)

            print ("In run model", self.prev_decoder_states)
            self.prev_time_step_data_second = self.add_placeholders_beamsearch_second(self.encoder_input_batch,self.model.initial_states)
            ...
            self.new_state_values_second = self.model.model.decode_one_step_second_decoder(self.encoder_input_batch, self.memory_states,
                                          ...
                                          content_weights=self.content_weights,content_prop_weights=None,word_indices=self.prev_time_step_data_second["words"],prop_indices= None,is_inference=True,var_name="new_decoder", is_prev_decoder=False)
            print ("<=>="*30)
            ...
            if self.config["Hyperparams"].get("use_pos_loss") == "True":
                self.loss_pos_op = loss_op(self.logits_pos, self.label_pos_placeholder, self.weights_placeholder, is_copy = (self.config["Decoder"]["decoder_type"] == "copy"), 
                          batch_size = int(self.config["Hyperparams"]["batch_size"]))
            ...
            if self.config["Hyperparams"]["use_only_rl"] == "True":
                total_loss = self.loss_reinforce_op
            elif self.config["Hyperparams"]["rl_plus_crossent"] == "True":
                total_loss  = (1 - self.rl_lambda_value) * self.loss_op + (self.rl_lambda_value) * self.loss_reinforce_op
            if self.config["Hyperparams"]["use_only_rl"] == "True" or self.config["Hyperparams"]["rl_plus_crossent"] == "True" :    
            # Add to the Graph the Ops that calculate and apply gradients.
                self.train_op = self.train_op_function(total_loss, self.learning_rate_placeholder, float(self.config["Hyperparams"]["gradient_clip"]))

            if self.config["POS"]["use_pos_loss"] == "True":
                lambda_value = float(self.config["POS"]["lambda_value"])
                loss_temp = (1- lambda_value) * self.loss_op + lambda_value * self.loss_pos_op
            else:
                loss_temp = self.loss_op
    ...
    def print_titles_in_files_beamsearch(self, sess, data_set, vocab, epoch = 1000):
        ...
        for step in xrange(steps_per_epoch):
            ...
            hyps, attn_weights, tokens_predicted, encoder_outputs, encoder_state, query_outputs, query_state, prev_coverage_vec_different = self.run_beam_search(sess, vocab, self.encoder_input_batch, encoder_inputs,self.query_input_batch, query_inputs)
            #shapes = [i.shape for i in prev_coverage_vec_different[0]]
            #print (shapes)
            #prev_coverage_vec_different = [np.sum(np.stack(i, axis=1), axis=1) for i in prev_coverage_vec_different]
            
            print(prev_coverage_vec_different)
            hyps_second,attn_weights_second = self.run_beam_search_2nd( sess, vocab, self.encoder_input_batch, encoder_inputs, 
                         ...
                         decoder_inputs = decoder_inputs, prev_coverage_vec_different=prev_coverage_vec_different)
    ...
    def add_placeholders_beamsearch(self,encoder_input_batch ,initial_states_place):
        ...
        prev_time_step_data["prev_coverage"]  = tf.zeros((batch_size*beam_size, int(self.config["Encoder"]["max_sequence_length"])))
        prev_time_step_data["prev_coverage_between_decoders"]  = tf.zeros((batch_size*beam_size, int(self.config["Decoder"]["max_prop_steps"])-1))

        masked_weights = tf.zeros(shape=(batch_size * beam_size,) , dtype=tf.int32)
        post_first_decoder_pl = tf.zeros(shape=(batch_size,int(self.config["Decoder"]["max_sequence_length"])-1) , dtype=tf.int32)
        prev_attention_weights = tf.zeros(shape=(batch_size,int(self.config["Decoder"]["max_sequence_length"])-1, int(self.config["Encoder"]["max_sequence_length"]) ))
        print ("In add placeholder mem states shape query {} state shape{}".format(memory_states,query_state))

        return memory_states,prev_time_step_data,query_state, masked_weights, content_weights, post_first_decoder_pl, prev_attention_weights
    ...
    def add_placeholders_beamsearch_second(self,encoder_input_batch ,initial_states_place):
        ...
        attns_state_different = tf.zeros((batch_size*beam_size, 2*int(self.config["Encoder"]["hidden_size"]) ), dtype=tf.float32)
        ...
        if self.config["Question_encoder"]["use_question_encoder"] == "True" and self.config["Question_encoder"]["word_concat"] == "True":
            ...
            prev_decoder_states = tf.zeros(shape=(batch_size*beam_size, int(self.config["Decoder"]["max_sequence_length"])-1, new_size ), dtype=tf.float32)
            ...
            prev_decoder_states = tf.zeros(shape=(batch_size*beam_size, int(self.config["Decoder"]["max_sequence_length"])-1, 2*int(self.config["Question_encoder"]["hidden_size"]) ), dtype=tf.float32)
        ####   Initial States when Question_encoder = False
            ...
            prev_decoder_states = tf.zeros(shape=(batch_size*beam_size, int(self.config["Decoder"]["max_sequence_length"])-1, int(self.config["Embedding"]["word_embedding_size"])), dtype=tf.float32)
        ...
        prev_time_step_data["prev_coverage"]  = tf.zeros((batch_size*beam_size, int(self.config["Encoder"]["max_sequence_length"])))
        prev_time_step_data["prev_coverage_vec_different"] = tf.zeros((batch_size*beam_size,int(self.config["Encoder"]["max_sequence_length"])))
        prev_time_step_data["prev_coverage_between_decoders"]  = tf.zeros((batch_size*beam_size, int(self.config["Decoder"]["max_prop_steps"])-1))
        ...
        masked_weights = tf.zeros(shape=(batch_size * beam_size,) , dtype=tf.int32)
        post_first_decoder_pl = tf.zeros(shape=(batch_size * beam_size,int(self.config["Encoder"]["max_sequence_length"])) , dtype=tf.int32)

        masked_weights_props = tf.zeros(shape=(batch_size * beam_size,int(self.config["Decoder"]["max_sequence_length"])-1) , dtype=tf.int32)
        prev_decoder_mean_attn = tf.zeros(shape=(batch_size * beam_size,int(self.config["Encoder"]["max_sequence_length"])) , dtype=tf.float32)
        question_gate = tf.zeros(shape=(batch_size * beam_size,),dtype=tf.float32)
    ...
    def run_beam_search(self, sess, vocab, encoder_input_batch, encoder_inputs, query_input_batch, query_inputs):
        ...
        beam_size = int(self.config["BeamSearch"]["beam_size"])
        #print ("Preksha list", [(i, len(encoder_inputs[i])) for i in encoder_inputs]) 
        ...
        # temp_feed_dict = {self.model.encoder_state: encoder_state}
        # initial_states = sess.run(self.model.initial_states, feed_dict=temp_feed_dict)
        ...
        hyps_batch =  [ [ Hypothesis(tokens=[vocab.encode_word("<s>")],
                           ...
                           content_weights = encoder_inputs["content_weights"][i],
                           words = encoder_inputs["word"][i],
                           attn_values = [],
        ...
        #   prev_time_step_data_val["property_state"] = np.asarray(prop_state)
          ...
          prev_content_weights = self.prev_time_step_data["content_weights"]
          prev_words = self.prev_time_step_data["words"]
	  ...
	  if self.config["Query"]["use_query"] == "True":
          ...
          #print (memory_states.shape, query_states.shape)
          #for k in encoder_inputs:
          #    print (k, encoder_inputs[k].shape)

          #for k in feed_dict:
          #    if isinstance(feed_dict[k],bool) or isinstance(feed_dict[k],float):
          #        a = None
          #    else:
          #        a = feed_dict[k].shape

          #    print (k, a)
          #for k in self.new_state_values:
          #    print (k, self.new_state_values[k])
          new_time_step_data = {}
        ...
        #tokens_predicted = np.asarray(tokens_predicted)[:,1:].tolist()
        token_predicted,_ = self.dataset.pad_data(tokens_predicted,int(self.config["Decoder"]["max_sequence_length"])-1)
        ...
        for i in top_sentences:
           ...
           attention_values.append(i.attn_dists)
...
def main():
    ...
    if (len(sys.argv) == 4 and sys.argv[3] == "inference"):
